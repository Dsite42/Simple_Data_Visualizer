...
class Plot3DAnalysis(BaseAnalysis):
	def __init__(self, main_app):
		super().__init__()
		self.main_app = main_app
		self.plot_title = tk.StringVar(value = "3D Plot")
		self.title_x_axis = tk.StringVar()
		self.title_y_axis = tk.StringVar()
		self.title_z_axis = tk.StringVar()
	...
	def create_plotly_plot(self, refresh_plot):
		...
		if len(selected_columns) < 1 or (len(selected_columns) == 1 and self.main_app.x_axis_combobox.get() == ""):
			...
			return
		if self.z_axis.get() == "":	
			messagebox.showinfo("Information", "Select a z-axis")
			return		

		# Create a Plotly 3D plot
		plot_args = self.create_plot_args(self.main_app.df)
		...
		if self.title_x_axis.get():
			fig.update_layout(scene=dict(xaxis=dict(title=self.title_x_axis.get())))
		if self.title_y_axis.get():
			fig.update_layout(scene=dict(yaxis=dict(title=self.title_y_axis.get())))
		if self.title_z_axis.get():
			fig.update_layout(scene=dict(zaxis=dict(title=self.title_z_axis.get())))
	...
	def show_plot3d_plot(self, refresh_plot):
		if self.main_app.use_plotly.get() == False or self.main_app.multiplot.get() == True:
			messagebox.showinfo("Information", "3D Plot is only available with Plotly and not in Multiplot or seaborn")
	...
	def init_ui(self):
		...
		for widget in self.main_app.plot3d_tab_frame.winfo_children():
			widget.destroy()
   

		# Frame for buttons
		button_frame = tk.Frame(self.main_app.plot3d_tab_frame)
		button_frame.pack(padx=5, pady=5, anchor='c')

		# Button to show 3D Plot in new window
		show_plot_button = tk.Button(button_frame, text="Show Plot", command=lambda: self.show_plot3d_plot(False))
		show_plot_button.grid(row=0, column=0, padx=5)
  
		# Button to refresh 3D Plot in same window
		show_plot_button = tk.Button(button_frame, text="Refresh Plot", command=lambda: self.show_plot3d_plot(True))
		...
		main_plot_arguments_frame = tk.Frame(self.main_app.plot3d_tab_frame)
		...
		# Entry for z-axis label
		z_axis_label = tk.Label(main_plot_arguments_frame, text="z-axis title:")
		z_axis_label.grid(row=3, column=0, padx=5)
		z_axis_entry = tk.Entry(main_plot_arguments_frame, textvariable=self.title_z_axis)
		z_axis_entry.grid(row=3, column=1, padx=5)
		# Entry for plot with
		plot_with_label = tk.Label(main_plot_arguments_frame, text="plot with:")
		plot_with_label.grid(row=4, column=0, padx=5)
		plot_with_entry = tk.Entry(main_plot_arguments_frame, textvariable=self.plot_with)
		plot_with_entry.grid(row=4, column=1, padx=5)
		# Entry for plot hight
		plot_hight_label = tk.Label(main_plot_arguments_frame, text="plot hight:")
		plot_hight_label.grid(row=5, column=0, padx=5)
		plot_hight_entry = tk.Entry(main_plot_arguments_frame, textvariable=self.plot_hight)
		plot_hight_entry.grid(row=5, column=1, padx=5)

		# Frame for plot arguments
		self.plot_arguments_frame = tk.Frame(self.main_app.plot3d_tab_frame)
		self.plot_arguments_frame.pack(padx=5, pady=5, fill='x')
		# z_axis
		self.z_axis_label = tk.Label(self.plot_arguments_frame, text="Z-axis:")
		self.z_axis_label.grid(row=0, column=0, padx=5, pady=5, sticky='w')
		self.z_axis_combobox = ttk.Combobox(self.plot_arguments_frame, textvariable=self.z_axis)
		self.z_axis_combobox.grid(row=0, column=1, padx=5, pady=5, sticky='w')
		# Kind
		self.kind_label = tk.Label(self.plot_arguments_frame, text="Kind:")
		self.kind_label.grid(row=1, column=0, padx=5, pady=5, sticky='w')
		self.kind_combobox = ttk.Combobox(self.plot_arguments_frame, textvariable=self.kind)
		self.kind_combobox.grid(row=1, column=1, padx=5, pady=5, sticky='w')
		# Hue
		...
		self.size_label.grid(row=3, column=0, padx=5, pady=5, sticky='w')
		self.size_combobox = ttk.Combobox(self.plot_arguments_frame, textvariable=self.size)
		self.size_combobox.grid(row=3, column=1, padx=5, pady=5, sticky='w')
		# Style
		self.style_label = tk.Label(self.plot_arguments_frame, text="Style:")
		self.style_label.grid(row=4, column=0, padx=5, pady=5, sticky='w')
		self.style_combobox = ttk.Combobox(self.plot_arguments_frame, textvariable=self.style)
		self.style_combobox.grid(row=4, column=1, padx=5, pady=5, sticky='w')
